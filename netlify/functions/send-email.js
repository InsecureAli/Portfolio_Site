exports.handler = async (event) => {
    // 1. CORS Headers (Allows your frontend to talk to your backend)
    const headers = {
    ...
    // 3. Only allow POST requests for actual form submissions
    ...
    try {
        ...
        // Set expiration for 1 hour (3600 seconds) on the first message
        if (count === 1) {
        ...
        // 6. Send to EmailJS (Includes Private Key for Strict Mode)
        const emailjsRes = await fetch('https://api.emailjs.com/api/v1.0/email/send', {
            ...
            body: JSON.stringify({
                ...
                accessToken: process.env.EMAILJS_PRIVATE_KEY, // Required for secure server environments
                template_params: {
        ...
        // 7. Return success or failure to the frontend
        if (emailjsRes.ok) {
